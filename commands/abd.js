...
const processId = async function(bot, chat) {
    ...
    if (!fs.existsSync(`./files/abd/${siteId}.json`)) {
        fs.writeFileSync(`./files/abd/${siteId}.json`, JSON.stringify({
            siteId: siteId,
            siteName: siteName
        }, null, 2));
    }

    if (!fs.existsSync(`./files/abd/${siteId}`)) {
        fs.mkdirSync(`./files/abd/${siteId}`);
    }
    ...
    const exampleFolder = './files/example/ABD/';
    ...
    list.forEach(async file => {
        ...
        await bot.sendMessage(chat.from, media, {id: siteId}, processFile);
    });
}
...
const processFile = async function(bot, chat) {
    ...
    const site = JSON.parse(fs.readFileSync(`./files/abd/${id}.json`));
    
    if (chat.hasMedia) {
        if (!site.files) {
            site.files = [];
        }
        if (!fs.existsSync(`./files/abd/${id}`)) {
            fs.mkdirSync(`./files/abd/${id}`);
        }
...
const done = async function(bot, from) {
    await bot.sendMessage(from, `ABD has been saved.\nThank you!`);
