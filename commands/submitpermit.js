...
const processId = async function(bot, chat) {
    ...
    if (!fs.existsSync(`./files/permit/${siteId}.json`)) {
        fs.writeFileSync(`./files/permit/${siteId}.json`, JSON.stringify({
            siteId: siteId,
            file: {}
        }, null, 2));
    }
    
    return await chat.sendMessage(chat.from, `Please attach permit file for ${siteId} or *!cancel* to back to main menu`, {siteId: siteId}, processFile);
}
...
const processFile = async function(bot, chat) {
    ...
    const site = JSON.parse(fs.readFileSync(`./files/permit/${id}.json`));
...
const done = async function(bot, chat) {
    await chat.sendMessage(chat.from, `Permit file has been saved.\nThank you!`);
