...
const processId = async function(bot, chat) {
    ...
    if (!fs.existsSync(`./files/topologi/${siteId}.json`)) {
        fs.writeFileSync(`./files/topologi/${siteId}.json`, JSON.stringify({
            siteId: siteId,
            file: {}
        }, null, 2));
    }
    
    return await chat.sendMessage(chat.from, `Please attach topologi file for ${siteId} or *!cancel* to back to main menu`, {siteId: siteId}, processFile);
}
...
const processFile = async function(bot, chat) {
    ...
    const site = JSON.parse(fs.readFileSync(`./files/topologi/${id}.json`));
